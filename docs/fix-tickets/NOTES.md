...
8. **血缘可追溯 ≠ 血缘可继承度量**。工单 08 的不变量 I4 只要求 `parent_scene_id`
   ...
   已加 `orchestrator._lineage_cutoff`：比对 `scene.restore_snapshot_id` 与父场景自己记的
   `snapshot_id_before` 字段，命中则整条跳过那一场。两处易错都踩过一遍：
   **判据不能解析快照 `label` 前缀**（`before:{场景名}`，场景名可含冒号）；
   **必须 `continue` 而非 `break`**，否则更早的真实历史会被连带丢掉（已有单测守住）。
   **复用别人立的不变量前，先确认它承诺的强度够不够你要的语义。**
