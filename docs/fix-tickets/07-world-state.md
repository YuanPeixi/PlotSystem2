...
> 查出的五个洞（并发覆盖、保留字被顶替、预算绕过、分叉窗口、键带换行）。
> **按本文原样再实现一遍会破坏契约**，请以 `CLAUDE.md` 与 NOTES 为准。
>
> ⚠️ **§5 第 1 条的真实 LLM 端到端（连跑两场）仍未执行**（需凭据），自动化用例用的是构造的 delta。
> §5 第 3 条要按分叉语义理解：rollback 会新建分支，应查询**新分支**的 world-state，
> 来源分支的世界变量本来就不会被改动。
