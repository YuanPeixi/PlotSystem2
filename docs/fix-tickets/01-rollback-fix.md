...
## 7. 回归项：重演场景不应携带 snapshot_id_before

**状态**：✅ 已修复（同分支后续 commit，见 `tests/test_orchestrator.py` 中
`assert new_scene.snapshot_id_before == ""` 断言）。

Code Review 指出：最初的实现在构造回滚重演的新 `Scene` 时，把 `snapshot_id_before` 直接设为
`target`（恢复用的快照 ID）。但 `SceneEngine.run()`（`backend/scene_engine/engine.py`）只有在
`scene.snapshot_id_before` 为空时才会创建"模拟前快照"，否则直接复用已有值、跳过创建。这意味着
重演场景永远不会为"恢复状态 + `new_initial_conditions`"之后的最新状态打一份快照，
`snapshot_id_before` 字段名不副实，后续再次回滚到这个"重演场景之前"时会拿到错误的状态。

修复：新场景创建时 `snapshot_id_before` 留空（`""`），让 `SceneEngine.run()` 在真正开始重演时
基于恢复后（并应用 `new_initial_conditions`）的状态自行创建一份新的快照。
