...
5. 终止后做收尾固化（`_consolidate_all`：`consolidate(force=True)`，唯一写入点在第4步，不重复写入）
   ...
   **三者顺序均不可颠倒**：若先打快照再固化，快照里的短期缓冲会带着"即将被固化"的原始文本，
   ...
   补事件摘要**（`replay_episodic`，只补摘要不写缓冲）——episodic 是独立内存层，
   不受水位线保护，按水位线切会让那段的重要事件在续跑后彻底消失；
...
<!-- 9fe7e99 适配：保留 evaluated_snapshot_id 的模型、反序列化、前端类型与
