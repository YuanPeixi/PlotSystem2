...
export const api = {
  ...
  listScenes: (id: string, branchId?: string) =>
    unwrap<Scene[]>(http.get(`/projects/${id}/scenes`, { params: { branch_id: branchId } })),
  ...
  getDecision: (sid: string) =>
    unwrap<DirectorDecision | null>(http.get(`/scenes/${sid}/decision`)),
  submitDecision: (sid: string, payload: Record<string, unknown>) =>
    unwrap<DirectorDecision>(http.post(`/scenes/${sid}/decision`, payload)),

  // 分支/快照
  getBranches: (id: string) => unwrap<BranchTree>(http.get(`/projects/${id}/branches`)),
  listSnapshots: (id: string) => unwrap<Snapshot[]>(http.get(`/projects/${id}/snapshots`)),
  forkBranch: (id: string, snapshotId: string, payload: Record<string, unknown>) =>
    unwrap<Branch>(
      http.post(`/snapshots/${snapshotId}/fork`, payload, { params: { project_id: id } }),
    ),
